...
function createResultScreen() {
    ...
    if (state.result === 'win') {
        ...
        div.innerHTML = `
            <h1 style="color: #ff3333;">Case Closed - Demoted</h1>
            <p style="color: #fff; font-size: 1.2rem; max-width: 800px; text-align: left;">
                You framed innocent people 3 times. Your disastrous mishandling of this case has resulted in the true killer escaping permanently.<br><br>
                Because of your absolute incompetence, you have been stripped of your rank and demoted to re-work a previous unsolved case.
            </p>
            <button class="primary-btn" onclick="goBackMystery()">Accept Demotion</button>
        `;
    } else {
        div.innerHTML = `
...
window.askQuestion = async function(suspectId, qIndex) {
    ...
    const responseText = await callGemini(target, questionText, powerUpUsed, currentMystery.setting, pastLog);
...
async function callGemini(suspect, question, powerUpId, settingContext, historyContext) {
    let powerUpContext = "";
