...
const state = {
    ...
    caseHistory: {}, // index -> { guessesRemaining: 0..3 }
    sessionFlags: {
        powerUpsUsed: new Set(),
        motivesRevealed: new Set(),
        startGuesses: 3,
        firstGuessFailed: false
    },
    settings: {
        musicOn: true,
        volume: 0.5,
...
const ACHIEVEMENTS = [
    { id: 'first_steps', name: "First Steps", desc: "Solve your very first case!", rarity: "Common", emoji: "👟" },
    { id: 'infallible', name: "Infallible Instincts", desc: "Solve a mystery with your very first accusation—no mistakes!", rarity: "Rare", emoji: "🎯" },
    { id: 'interrogation_expert', name: "Interrogation Expert", desc: "Exhaust all available questions with three different suspects in a single case.", rarity: "Uncommon", emoji: "🗣️" },
    { id: 'power_play', name: "Power Play", desc: "Use every type of power-up (Truth Serum, Intimidation, Charm) in one case.", rarity: "Uncommon", emoji: "⚡" },
    { id: 'streak_seeker', name: "Streak Seeker", desc: "Win 3 mysteries in a row without being dismissed.", rarity: "Uncommon", emoji: "🔥" },
    { id: 'speedy_sleuth', name: "Speedy Sleuth", desc: "Solve a case in under 2 minutes.", rarity: "Rare", emoji: "⏱️" },
    { id: 'notebook_detective', name: "Notebook Detective", desc: "Fill up the entire detective notebook (ask at least 15 questions in one case).", rarity: "Uncommon", emoji: "📝" },
    { id: 'mind_reader', name: "Mind Reader", desc: "Correctly accuse the killer without using any power-ups.", rarity: "Rare", emoji: "🧠" },
    { id: 'second_chances', name: "Second Chances", desc: "Lose on your first accusation, but solve the mystery before your last guess.", rarity: "Uncommon", emoji: "♻️" },
    { id: 'persistent_profiler', name: "Persistent Profiler", desc: "Interrogate at least 5 different suspects in one case.", rarity: "Common", emoji: "🕵️" },
    { id: 'perfect_season', name: "Perfect Season", desc: "Clear every case in 'EASY' mode without a single failed accusation.", rarity: "Epic", emoji: "🌟" },
    { id: 'no_stone_unturned', name: "No Stone Unturned", desc: "Ask every available question to every suspect in a case.", rarity: "Epic", emoji: "🪨" },
    { id: 'all_motives', name: "All the Motives", desc: "Reveal hidden truths—use power-ups strategically to unlock all suspect motives in one case.", rarity: "Rare", emoji: "🔍" },
    { id: 'back_from_brink', name: "Back From the Brink", desc: "Win a case when you have only one accusation remaining.", rarity: "Rare", emoji: "🧗" },
    { id: 'legendary_detective', name: "Legendary Detective", desc: "Achieve a win streak of 7 or more mysteries.", rarity: "Epic", emoji: "👑" },
    { id: 'completionist', name: "Completionist", desc: "Solve all 20 mysteries.", rarity: "Epic", emoji: "💎" }
];
...
async function loadMystery(index) {
    try {
        ...
        const res = await fetch(`${API_BASE}/api/mystery/${index}`, { headers });
...
async function createNewSession(index) {
    try {
        const sRes = await fetch(`${API_BASE}/api/start-game`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ mysteryIndex: index })
        });
        if (!sRes.ok) throw new Error('Could not initialize game session.');
    ...
    if (state.screen !== 'check_auth' && state.screen !== 'loading' && (state.screen !== 'start' || state.user)) {
        ...
        globalNav.innerHTML = `
            <div class="global-nav-links">
                <button class="primary-btn achievement-nav-btn" onclick="showAchievements()">
    ...
    if (user) {
        try {
            ...
            if (pDoc.exists()) {
                ...
                if (data.log !== undefined) state.log = data.log;
                if (data.achievements !== undefined) state.achievements = data.achievements;
                if (data.caseHistory !== undefined) state.caseHistory = data.caseHistory;
...
function loadLocalState() {
    try {
        ...
        if (saved) {
            ...
            state.caseHistory = data.caseHistory || {};
        }
...
function showAchievementToast(achievement) {
    ...
    toast.className = 'achievement-toast fade-in';
    toast.innerHTML = `
        <div class="toast-icon">${achievement.emoji}</div>
        <div class="toast-content">
            <div class="toast-title">Achievement Unlocked!</div>
            <div class="toast-name">${achievement.name}</div>
            <div class="toast-rarity">${achievement.rarity}</div>
        </div>
    `;
    document.body.appendChild(toast);
    
    // Play a subtle sound if possible
    const sfx = new Audio('https://assets.mixkit.co/active_storage/sfx/2013/2013-preview.mp3');
    sfx.volume = 0.3;
    if (state.settings.musicOn) sfx.play().catch(() => {});
...
function showAchievements() {
    ...
    modal.className = 'achievements-modal fade-in';
    modal.innerHTML = `
        <div class="achievements-content glass-panel">
            ...
            <div class="achievements-grid">
                ${ACHIEVEMENTS.map(a => {
                    ...
                    return `
                        <div class="achievement-card ${isUnlocked ? 'unlocked' : 'locked'} ${a.rarity.toLowerCase()}-rarity">
                            <div class="achievement-icon">${isUnlocked ? a.emoji : '🔒'}</div>
                            <div class="achievement-info">
                                <div class="achievement-name">${a.name}</div>
                                <div class="achievement-rarity-badge">${a.rarity}</div>
                            </div>
...
window.startGame = async function () {
    ...
    if (!await createNewSession(state.currentMysteryIndex)) return;
    ...
    // Reset session flags for achievements
    state.sessionFlags = {
        powerUpsUsed: [],
        motivesRevealed: [],
        startGuesses: 3,
...
window.usePowerUp = function (pId) {
    if (state.inventory[pId] > 0 && state.activePowerUp === null) {
        ...
        // Track for 'Power Play' achievement
        if (!state.sessionFlags.powerUpsUsed.includes(pId)) {
            state.sessionFlags.powerUpsUsed.push(pId);
        }
...
window.askQuestion = async function (suspectId, qIndex) {
    ...
    // Tracker for 'All the Motives'
    if (powerUpUsed && !state.sessionFlags.motivesRevealed.includes(target.id)) {
        state.sessionFlags.motivesRevealed.push(target.id);
    }
...
function checkMidGameAchievements() {
    // 1. Interrogation Expert: Exhaust questions for 3 suspects
    const maxQs = state.currentMysteryIndex < 4 ? 6 : (state.currentMysteryIndex < 8 ? 4 : 3);
    const exhaustedCount = Object.values(state.questionsAsked).filter(count => count >= maxQs).length;
    if (exhaustedCount >= 3) unlockAchievement('interrogation_expert');

    // 2. Persistent Profiler: Interrogate 5 suspects
    if (Object.keys(state.questionsAsked).length >= 5) unlockAchievement('persistent_profiler');

    // 3. Notebook Detective: 15+ questions
    const qCount = state.log.filter(l => l.type === 'qna').length;
    if (qCount >= 15) unlockAchievement('notebook_detective');

    // 4. Power Play: Use all 3 power-ups
    if (state.sessionFlags.powerUpsUsed.length === 3) unlockAchievement('power_play');

    // 5. No Stone Unturned: Every suspect exhausted
    const allExhausted = state.currentMystery.suspects.every(s => (state.questionsAsked[s.id] || 0) >= maxQs);
    if (allExhausted) unlockAchievement('no_stone_unturned');

    // 6. All the Motives: Every suspect had a motive revealed via power-up
    const allMotives = state.currentMystery.suspects.every(s => state.sessionFlags.motivesRevealed.includes(s.id));
    if (allMotives) unlockAchievement('all_motives');
}
    ...
    if (isConfirmed) {
        try {
            ...
            if (data.correct) {
                ...
                // Track Case History for 'Perfect Season'
                if (!state.caseHistory[state.currentMysteryIndex] || data.guessesRemaining > (state.caseHistory[state.currentMysteryIndex].guessesRemaining || 0)) {
                    state.caseHistory[state.currentMysteryIndex] = { guessesRemaining: state.guessesRemaining };
                }
                ...
                checkEndGameAchievements(true, secondsTaken);
                ...
                state.sessionFlags.firstGuessFailed = true;
...
function checkEndGameAchievements(won, secondsTaken) {
    if (!won) return;

    // 1. First Steps
    unlockAchievement('first_steps');

    // 2. Infallible Instincts: 3 guesses remaining on a win
    if (state.guessesRemaining === 3) unlockAchievement('infallible');

    // 3. Mind Reader: No power-ups used
    if (state.sessionFlags.powerUpsUsed.length === 0) unlockAchievement('mind_reader');

    // 4. Second Chances: Failed first, but won with 2 guesses left
    if (state.sessionFlags.firstGuessFailed && state.guessesRemaining > 1) unlockAchievement('second_chances');

    // 5. Speedy Sleuth: < 2 mins
    if (secondsTaken < 120) unlockAchievement('speedy_sleuth');

    // 6. Back From the Brink: Won with only 1 guess left
    if (state.guessesRemaining === 1) unlockAchievement('back_from_brink');

    // 7. Streak Seeker: 3 wins
    if (state.streak >= 3) unlockAchievement('streak_seeker');

    // 8. Legendary Detective: 7+ wins
    if (state.streak >= 7) unlockAchievement('legendary_detective');

    // 9. Completionist: Solve all 20 mysteries (0-19)
    const solvedCount = Object.keys(state.caseHistory).length;
    if (solvedCount >= 20 || state.currentMysteryIndex === 19) unlockAchievement('completionist');

    // 10. Perfect Season: Cases 0-3 solved without failures
    const easyIndices = [0, 1, 2, 3];
    const perfectEasy = easyIndices.every(idx => state.caseHistory[idx] && state.caseHistory[idx].guessesRemaining === 3);
    if (perfectEasy) unlockAchievement('perfect_season');
}
...
window.nextMystery = async function () {
    ...
    if (state.currentMysteryIndex < MYSTERY_COUNT - 1) {
        ...
        state.guessesRemaining = 3;
        state.mysteryStartTime = Date.now();
        state.log = []; // Clear interrogation log for the next case
        
        // Start fresh secure session for the next mystery
        if (!await createNewSession(state.currentMysteryIndex)) return;
