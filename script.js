...
function render() {
    ...
    if (state.user) {
        ...
        const displayName = state.user ? (state.user.displayName ? state.user.displayName.split(' ')[0] : 'Detective') : 'Guest';
        header.innerHTML = `
            <div class="header-main">
                <span>Detective ${displayName}</span>
                <div class="stat-pills">
                    <span class="stat-pill score-pill">SCORE: ${state.score}</span>
                </div>
            </div>
            <div class="header-items">
                <span class="stat-pill version-pill">${state.version}</span>
                <span class="stat-pill difficulty-pill">${state.currentMysteryIndex < 4 ? 'EASY' : (state.currentMysteryIndex < 8 ? 'MEDIUM' : 'HARD')} MODE</span>
                <span class="stat-pill streak-pill">STREAK: ${state.streak}🔥</span>
