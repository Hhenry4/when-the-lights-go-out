...
// ============================================================
// SESSIONS — In-memory store for game instances.
// ============================================================
const sessions = new Map();

// --- Production-Ready Session Cleanup (Runs every 10 minutes) ---
const SESSION_LIFETIME_MS = 3600000; // 60 Minutes
setInterval(() => {
  const now = Date.now();
  let cleaned = 0;
  for (const [id, session] of sessions.entries()) {
    const isExpired = now - session.startTime > SESSION_LIFETIME_MS;
...
// Middleware: Validates session for gameplay routes
function validateSession(req, res, next) {
  ...
  // Safe-check: ensure session exists and hasn't just been cleaned up
  if (!session) {
    return res.status(401).json({ error: 'Invalid or expired session. Please start a new case.' });
  }

  // Real-time expiry check (in case background cleanup hasn't run yet)
  if (Date.now() - session.startTime > 3600000) {
    sessions.delete(sessionId);
    return res.status(401).json({ error: 'Session has expired (60 minute limit).' });
  }

  if (session.status === 'completed') {
    return res.status(403).json({ error: 'This case is already closed. Please start a new investigation.' });
  }

  // Bind to request for downstream safety
  req.session = session;
...
let mysteries = [
  ...
  {
    "id": "m4",
    "title": "Penthouse Poisoning",
    "setting": "A high-stakes poker game ends in tragedy when the host collapses after taking a sip from a sealed bottle.",
    "killerId": "contractor",
    "suspects": [
      {
        "id": "victim",
...
app.post('/api/accuse', validateSession, (req, res) => {
  try {
    ...
    if (isCorrect) {
      session.status = 'completed'; // Lock the session once solved
      const killer = mystery.suspects.find(s => s.id === suspectId);
      return res.json({
        correct: true,
        killerId: mystery.killerId,
        hiddenTruth: killer ? killer.hiddenTruth : "The motive remains a mystery..."
      });
    }
...
app.post('/api/interrogate', validateSession, async (req, res) => {
  try {
    const { mysteryIndex, suspectId, question, powerUpId, history } = req.body;
    const session = req.session;

    // Security: Ensure they are interrogating the mystery they started
    const index = session.mysteryIndex;
    const mystery = mysteries[index];
    ...
    if (!session.questions[suspectId]) session.questions[suspectId] = 0;
    const askedCount = session.questions[suspectId];
    ...
    // Increment count only AFTER limit check
    session.questions[suspectId]++;
...
4. CONSISTENCY AND VARIETY: If "PREVIOUS INTERROGATION HISTORY" is provided above, you MUST remember your previous answers and NEVER contradict them. However, do NOT reuse previous responses or phrases; each answer should feel unique and progressive.
...
7. Respond in 2 to 4 spoken sentences. Provide enough detail to be informative and engaging (moderately long responses are encouraged), but do not be overly verbose. Do NOT include actions in asterisks, just the spoken text.
8. YOUR GOAL IS TO BE A FUN, CHALLENGING, BUT FAIR PART OF A GAME. Give the player enough information to form a theory.
9. PREVENT PLACEHOLDERS: NEVER use bracketed placeholders like "[Victim's Name]" in your responses. Speak naturally about "the victim" if no specific name is provided.
The Detective asks: "${question}"`;
    ...
    if (!data.candidates || !data.candidates[0] || !data.candidates[0].content) {
      ...
      return res.status(500).json({ error: "The suspect is refusing to speak (AI Error)." });
    }
