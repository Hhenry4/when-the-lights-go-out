...
setInterval(() => {
  ...
  if (cleaned > 0) console.log(`🧹 Session Cleanup: Removed ${cleaned} expired/finished sessions.`);
}, 600000);
...
// Returns a single mystery's public info by index (0-based) - Requires active session
app.get('/api/mystery/:index', validateSession, (req, res) => {
  ...
  // Security: Ensure session can only access its own assigned mystery
  if (index !== session.mysteryIndex) {
    return res.status(403).json({ error: 'Unauthorized mystery access. Session mismatch.' });
  }
